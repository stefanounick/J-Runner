...
namespace JRunner
{
    class HID
    {
            ...
            try
            {
                if (!(HID.BootloaderDetected))
                {
                    ...
                    if (deviceFound == true)
                    {
                        ...
                        if (success)
                        {
                            if (variables.debugme) Console.WriteLine("Bootloader attached -85");
                    ...
                    if (HID.BootloaderDetected)
                    {
                        ...
                        if (success)
                        {
                            ...
                            {
                                if (variables.debugme) Console.WriteLine("Bootloader attached.-117");
                            }
                            else
                            {
        ...
        public static Boolean FindBootloader()
        {
            ...
            Console.WriteLine("\nBootloader Attached");
            Console.WriteLine("Bootloader Version: {0:X}.{1:X}.{2:X}", version.ToString("X").Substring(0, 1), version.ToString("X").Substring(1, 1), version.ToString("X").Substring(2, 1));
            FindBootloader_WIN();
        ...
        {
            if (String.IsNullOrEmpty(variables.filename1)) { MessageBox.Show("File is missing"); return; }
            if (_deviceList.Count() != 0) _selectedDevice = _deviceList[0];
            ...
            if (_selectedDevice.IsConnected)
            {
                ...
                erase();
                Thread.Sleep(175);
                Console.Write("Writing Flash: ");
                if (write(ref pb)) Console.WriteLine("\nDone!");
                else { Console.WriteLine("Write Failed"); return; }
                Thread.Sleep(10);
                pb.Value = 0;
                _selectedDevice.CloseDevice();
                _selectedDevice.OpenDevice();
                Console.Write("Verifying Flash: ");
                if (verify(ref pb)) Console.WriteLine("\n Write Verified!");
